...
function RSAPKCS1V15_Decrypt(buffer, privateKey) {
    try {
        ...
        return (0, node_opcua_crypto_1.privateDecrypt_long)(buffer, privateKey, blockSize, node_opcua_crypto_1.PaddingAlgorithm.RSA_PKCS1_PADDING);
    }
    catch (err) {
        console.log("err = ", err);
        throw err;
...
function RSAOAEP_Decrypt(buffer, privateKey) {
    const blockSize = (0, node_opcua_crypto_1.rsaLengthPrivateKey)(privateKey);
    return (0, node_opcua_crypto_1.privateDecrypt_long)(buffer, privateKey, blockSize, node_opcua_crypto_1.PaddingAlgorithm.RSA_PKCS1_OAEP_PADDING);
}
// --------------------
...
function RSAPKCS1V15_Encrypt(buffer, publicKey) {
    try {
        ...
        return (0, node_opcua_crypto_1.publicEncrypt_long)(buffer, publicKey, keyLength, 11, node_opcua_crypto_1.PaddingAlgorithm.RSA_PKCS1_PADDING);
    }
    catch (err) {
        console.log(err);
        throw err;
...
function RSAOAEP_Encrypt(buffer, publicKey) {
    const keyLength = (0, node_opcua_crypto_1.rsaLengthPublicKey)(publicKey);
    return (0, node_opcua_crypto_1.publicEncrypt_long)(buffer, publicKey, keyLength, 42, node_opcua_crypto_1.PaddingAlgorithm.RSA_PKCS1_OAEP_PADDING);
}
...
function RSA_PSS_SHA2_256_Verify(buffer, signature, certificate) {
    ...
    return verify.verify({
        key: (0, node_opcua_crypto_1.toPem)(certificate, "CERTIFICATE"),
        padding: crypto_2.constants.RSA_PKCS1_PSS_PADDING,
        saltLength: crypto_2.constants.RSA_PSS_SALTLEN_DIGEST
    }, signature);
}
function RSA_OAEP_SHA2_256_Encrypt(buffer, publicKey) {
    const blockSize = (0, node_opcua_crypto_1.rsaLengthPublicKey)(publicKey);
...
function getOptionsForSymmetricSignAndEncrypt(securityMode, derivedKeys) {
    ...
    if (securityMode === node_opcua_service_secure_channel_1.MessageSecurityMode.SignAndEncrypt) {
        options = {
            ...
            encryptBufferFunc: (chunk) => (0, node_opcua_crypto_1.encryptBufferWithDerivedKeys)(chunk, derivedKeys),
